...
- 2026-05-24 — Completed Dashboard UI Phase 3: installed shadcn `card` + `badge`; built `src/components/dashboard/stats-cards.tsx` (4 tiles: items, collections, favorite items, favorite collections — counts from mock data), `recent-collections.tsx` + `collection-card.tsx` (newest-first grid, type-colored top accent, distinct-type icon row at the bottom), and `pinned-items.tsx` / `recent-items.tsx` sharing an `item-card.tsx` (left accent strip in the item type color, type icon, pin + favorite badges, 10-item cap on recent, pinned section hides when empty); composed all four sections into `src/app/dashboard/page.tsx`. `ItemCard` resolves its Lucide icon via `React.createElement` instead of a `const Icon = …` binding to satisfy the `react-hooks/static-components` lint rule. Build passes.
